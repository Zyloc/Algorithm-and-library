...
lli constructSegmentTreeUtil(vector<lli> &v,lli* segmentTree,lli start, lli end, lli pos){
	...
	lli mid((start+end)/2);
	segmentTree[pos] = (constructSegmentTreeUtil(v,segmentTree,start,mid,2*pos+1))%MOD+(constructSegmentTreeUtil(v,segmentTree,mid+1,end,2*pos+2))%MOD;
...
lli updateSegmentTreeUtil(lli* segmentTree,lli segmentStart,lli segmentEnd,lli qstart,lli qend,lli value,lli pos){
	...
	if (segmentStart == segmentEnd){
		segmentTree[pos] = (segmentTree[pos]%MOD*segmentTree[pos]%MOD*segmentTree[pos]%MOD)%MOD;
		return segmentTree[pos]%MOD; 
	}
